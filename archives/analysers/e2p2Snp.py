...
def get_e2p2_ecNumbers(e2p2_pred_dic):
	return [ x for x in set(reduce(lambda l1, l2: l1 + l2, e2p2_pred_dic.values())) if x != "NA"]

def write_list_ecNumbers(e2p2_list_ecNumbers, ref_list_ecNumbers):
	file = open("e2p2_ecNumbers.txt", 'w')
	for ec in e2p2_list_ecNumbers:
	  	file.write("%s\n" % ec)
	file.close()
...
def get_Snp_for_ecNumbers(e2p2_pred_dic, ref_dic, level):
	dic_ecNumbers = {}
	for protID in ref_dic:
		if protID in e2p2_pred_dic and e2p2_pred_dic[protID] != ['NA'] and ref_dic[protID] != ['NA']:
			...
			if (4 - int(level) > 0):
				list_ECs_e2p2_class = ['.'.join(ec[0].split('.')[:-4 +int(level)]) for ec in list_ECs_e2p2]
				list_ECs_e2p2_class = list(set(list_ECs_e2p2_class))     # to avoid redondancies
				...
				list_ECs_e2p2_class = list_ECs_e2p2
				list_ECs_uniprot_class = list_ECs_uniprot
			
			for ec in list_ECs_e2p2_class:
			...
			for ec in list_ECs_uniprot_class:
				if ec not in list_ECs_e2p2_class:
			...
			if (4 - int(level) > 0):
				list_ECs_e2p2_class = ['.'.join(ec[0].split('.')[:-4+int(level)]) for ec in list_ECs_e2p2]
				list_ECs_e2p2_class = list(set(list_ECs_e2p2_class))     # to avoid redondancies
			else: list_ECs_e2p2_class = list_ECs_e2p2
			for ec in list_ECs_e2p2_class:
				if ec in dic_ecNumbers: dic_ecNumbers[ec][1]+= 1
				else: dic_ecNumbers[ec] = [0,1,0]		
		elif protID in e2p2_pred_dic and e2p2_pred_dic[protID] == ['NA'] and ref_dic[protID] != ['NA']:
		...
		elif protID in e2p2_pred_dic and e2p2_pred_dic[protID] == ['NA'] and ref_dic[protID] != ['NA']: continue
		...
		elif protID not in e2p2_pred_dic and ref_dic[protID] == ["NA"]: continue
	
	file = open("e2p2_snp.txt", "w")
...
if __name__ == '__main__':
	...
	if args.fasta is None or args.e2p2 is None or args.sp is None or args.level is None: 
		parser.print_help()
		exit(0)
	#
	ref_dic 			 = get_sp_data(args.fasta, args.sp)
	e2p2_pred_dic = get_e2p2_data(args.e2p2)
	#get_Snp_for_ecNumbers(e2p2_pred_dic, ref_dic, args.level)
	#
	ref_ecNumbers = get_ref_ecNumbers(ref_dic)
	e2p2_ecNumbers = get_e2p2_ecNumbers(e2p2_pred_dic)
	write_list_ecNumbers(e2p2_ecNumbers, ref_ecNumbers)
