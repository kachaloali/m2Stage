...
class convPLEAP:
	...
	def check_type_file(self):
		...
		elif Error == "" and "T" in output and "F" in output : return True
		elif Error == "" and ("T" in output or "F" in output): return True	
		return typeFile
...
if __name__ == '__main__':
	from convPLEAP import convPLEAP
	...
	parser.add_argument("-f", "--file", 
						...
						help = "The Pleap output file name")
	parser.add_argument("-o", "--out",
		                dest = "output",
		                type = str,
		                default = "[Input filename without its extension]_pleap.conv",
		                help = "The output file name")
	args = parser.parse_args()
	if args.file is None: 
			parser.print_help()
			exit(0)
	default_output = "[Input filename without its extension]_pleap.conv"
	if args.output == default_output: args.output = get_output_name(args.file) + "_pleap.conv"
	convPLEAP(args.file, args.output).convert_file()
